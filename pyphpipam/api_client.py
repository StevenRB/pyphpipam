...
class IpAdmin(object):
    ...
    def login(self):
        call = requests.post(self.base_url + 'user/', auth=(self.__username, self.__password), verify=self.cert_path)
    ...
    def get_subnet_id(self, subnet, simple=False):
        call = requests.get(self.base_url + 'subnets/cidr/{}/'.format(subnet), headers=self.headers,
    ...
    def get_next_subnet(self, parent_id, mask, simple=False):
        call = requests.get(self.base_url + 'subnets/{}/first_subnet/{}/'.format(parent_id, mask), headers=self.headers,
                            verify=self.cert_path)
        return simple_return(call, simple)

    # Retrieves all addresses in a subnet. Returns a list of dictionaries
    def get_all_addresses(self, subnet_id, simple=False):
        call = requests.get(self.base_url + 'subnets/{}/addresses/'.format(subnet_id), headers=self.headers,
                            verify=self.cert_path)
        return simple_return(call, simple)
    ...
    def create_next_subnet(self, parent_id, mask, desc, simple=False):
        ...
        call = requests.post(self.base_url + 'subnets/{}/first_subnet/{}/'.format(parent_id, mask), params=data,
                             headers=self.headers, verify=self.cert_path)
        return simple_return(call, simple)

    # Retrieves an address's ID. Arg is a IP
    def get_address_id(self, ip_add, simple=False):
        call = requests.get(self.base_url + 'addresses/search/{}/'.format(ip_add), headers=self.headers,
                            verify=self.cert_path)
        return simple_return(call, simple)
    ...
    def create_next_ip(self, parent_id, desc, simple=False):
        data = {"description": "{}".format(desc)}
        call = requests.post(self.base_url + 'addresses/first_free/{}/'.format(parent_id), params=data,
                             headers=self.headers, verify=self.cert_path)
        return simple_return(call, simple)
    ...
    def create_subnet(self, parent_id, network, mask, desc, simple=False):
        data = {"masterSubnetId": "{}".format(parent_id),
                "description": "{}".format(desc),
                "subnet": "{}".format(network),
                "mask": "{}".format(mask),
                "sectionId": 5
                }
        call = requests.post(self.base_url + 'subnets/', params=data,
                             headers=self.headers, verify=self.cert_path)
        return simple_return(call, simple)

    # Creates specific IP in the parent subnet
    def create_address(self, parent_id, ip, desc, simple=False):
        data = {"subnetId": "{}".format(parent_id),
                "description": "{}".format(desc),
                "ip": "{}".format(ip),
                "note": "Created via API"}
        call = requests.post(self.base_url + 'addresses/', params=data, headers=self.headers, verify=self.cert_path)
        return simple_return(call, simple)

    # Updates the description on a specific IP
    def update_address(self, address_id, desc, simple=False):
        data = {"description": "{}".format(desc)}
        call = requests.patch(self.base_url + 'addresses/{}/'.format(address_id), params=data, headers=self.headers,
                              verify=self.cert_path)
        return simple_return(call, simple)
    ...
    def update_subnet(self, subnet_id, desc, simple=False):
        data = {"description": "{}".format(desc)}
        call = requests.patch(self.base_url + 'subnets/{}/'.format(subnet_id), params=data, headers=self.headers,
                              verify=self.cert_path)
        return simple_return(call, simple)
    ...
    def create_domain(self, name, simple=False):
        ...
        call = requests.post(self.base_url + 'l2domains/', params=data,
                             headers=self.headers, verify=self.cert_path)
        return simple_return(call, simple)

    # Requests all L2Domains
    def getdomain(self):
        call = requests.get(self.base_url + 'l2domains/', 
                            headers=self.headers, verify=self.cert_path)
        simple = False
        return simple_return(call, simple)
    ...
    def create_vlan(self, name, number, domain=1, simple=False):
        ...
        call = requests.post(self.base_url + 'vlan/', params=data, headers=self.headers, verify=self.cert_path)
        return simple_return(call, simple)
    ...
    def create_location(self, name, address, simple=False):
        ...
        call = requests.post(self.base_url + 'tools/locations/',
                             headers=self.headers, params=data, verify=self.cert_path)
        return simple_return(call, simple)

    # Deletes a subnet referenced by subnet ID. Will not delete child subnets
    # !!! DOES NOT ASK FOR VERIFICATION !!!
    def delete_subnet(self, subnet_id):
        call = requests.delete(self.base_url + 'subnets/{}/'.format(subnet_id), headers=self.headers,
                               verify=self.cert_path)
    ...
    def delete_subnet_ips(self, subnet_id):
        call = requests.delete(self.base_url + 'subnets/{}/truncate'.format(subnet_id), headers=self.headers,
                               verify=self.cert_path)
        info = json.loads(call.text)
        return info
    ...
    def delete_address(self, address_id):
        call = requests.delete(self.base_url + 'addresses/{}/'.format(address_id), headers=self.headers,
                               verify=self.cert_path)
        info = json.loads(call.text)
        return info
